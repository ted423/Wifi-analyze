...
BEGIN_MESSAGE_MAP(CwlanDlg, CDialogEx)
	...
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON3, &CwlanDlg::OnBnClickedButton3)
...
void CwlanDlg::wifiquantity()
{
	...
	if (pWLAN_AVAILABLE_NETWORK_LIST[0].Network[i].bSecurityEnabled==1)
	{
		...
		hSubItem = m_display.InsertItem(temp,hItem);
	for (int j=0,k=0;j<pWlanBssList[0].dwNumberOfItems;j++)
	{
...
{
	UpdateData(TRUE);
	CFile file(".\\ssid.dat",CFile::modeReadWrite);
	CFile mfile(".\\mac.dat",CFile::modeReadWrite);
	char pbuf[999];
...
void CwlanDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	if (nIDEvent == 1){CwlanDlg::wifiquantity();}
	CDialogEx::OnTimer(nIDEvent);
}


void CwlanDlg::OnBnClickedButton3()
{
	// TODO: 在此添加控件通知处理程序代码
	KillTimer(1);
}
...
void CwlanDlg::OnBnClickedButton5()
{
	...
	htemp=NULL;
	DeleteFile(".\\ssid.dat");
		CFile file(".\\ssid.dat",CFile::modeCreate|CFile::modeReadWrite);
	htemp=m_mistrustssid.GetRootItem();
	temp=m_mistrustssid.GetItemText(htemp);
...
DeleteFile(".\\mac.dat");
		CFile mfile(".\\mac.dat",CFile::modeCreate|CFile::modeReadWrite);
		htemp=m_mistrustmac.GetRootItem();
		temp=m_mistrustmac.GetItemText(htemp);
		 i=temp.GetLength();
